...
from plots import (
    plot_weights_labeled, plot_filter_profile, plot_combined_response,
    plot_time_iq_complex, plot_constellation,
)
from math import erfc as _erfc_scalar  # Q(x) sin SciPy
...
class equalizerSimulator:
    def __init__(self,
        ...
        self.L_EQ         = L_EQ
        self.PART_N       = PART_N
        self.CENTER_TAP   = (PART_N)//2 if CENTER_TAP is None else CENTER_TAP
        # mu / switch
        self.MU               = MU
    ...
    def plot(self,
        ...
        if weights:   plot_weights_labeled(W_hist, center_index=self.CENTER_TAP)
        if profile:   plot_filter_profile(w_fin, center_index=self.CENTER_TAP)
        if conv:      plot_combined_response(w_fin, self.H_TAPS)
        if time_in:   plot_time_iq_complex(self.x_n, self.N_PLOT, "Salida del canal: x_n")
        if time_out:
...
def plot_ber_curve(results, title="BER vs SNR"):
    ...
    # teoria suave 0..15 dB
    x_dense_esn0 = np.linspace(0, 12, 500)
    ...
    plt.xlabel("SNR")   # correcto segun channel()
    plt.ylabel("BER")
    plt.xlim(0, 12)
    plt.ylim(1e-7, 1)
    plt.title(title)
