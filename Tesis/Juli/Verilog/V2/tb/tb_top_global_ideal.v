...
module tb_top_global_ideal;
  ...
  // ======================
  // CONDICION REAL DE "AVANCE"
  // ======================
  // Preferido: si en tu top dejaste el wire drive_valid, úsalo.
  // Si no existe, usa tx_en_w (cuando PRBS avanza) o os_in_ready (si rx_in_valid=1).
  wire adv;
  assign adv =
      // si existe:
      (dut.drive_valid === 1'b1) ? 1'b1 :
      // fallback 1:
      (dut.tx_en_w     === 1'b1) ? 1'b1 :
      // fallback 2:
      (dut.os_in_ready_w === 1'b1);
  ...
  // ======================
  // Captura bloque "curr" REAL (N_PART samples aceptados)
  // ======================
  task drive_and_capture_block;
    ...
    begin
      ...
      while (idx < N_PART) begin
        ...
        if (adv) begin
          // Capturá exactamente lo que alimenta el OS.
          // Ideal: rx_in_I/Q si existen como wires internos. Si no, usa ch_out_I/Q.
          currI[idx] = (dut.rx_in_I !== 1'bx) ? dut.rx_in_I : ch_out_I;
          currQ[idx] = (dut.rx_in_Q !== 1'bx) ? dut.rx_in_Q : ch_out_Q;
          idx = idx + 1;
        end
      end
    end
  ...
  task capture_and_vm_frame;
    ...
    reg seen_start;
    begin
      // esperar ifft_start si existe; si no, esperar primer ifft_valid
      t = 0;
      seen_start = 0;
