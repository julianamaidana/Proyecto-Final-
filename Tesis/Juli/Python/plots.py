...
def plot_ber_vs_mu(mus, bers, title="BER vs μ"):
    """
    Grafica BER (eje Y log) vs μ (eje X log).
    """
    plt.figure(figsize=(7, 4.5))
    plt.loglog(mus, bers, 'o-', mfc='none') # Usar log-log
    plt.xlabel("Paso de adaptación (μ)")
    plt.ylabel("BER")
    plt.title(title)
    plt.grid(True, which="both", linestyle=":")
    plt.tight_layout()
    # plt.show() se llama al final del script principal