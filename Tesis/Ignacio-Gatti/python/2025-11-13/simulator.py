...
from pbfdaf_lms import FFE_Engine, ffe_lms_freq_iq
from plots import (
...
class equalizerSimulator:
    ...
    def equalize(self):
        ...
        # Copiar historial de pesos desde el motor FFE
        if hasattr(self.ffe_motor, '_W_histI'):
            n_hist = min(self.ffe_motor._W_histI.shape[0], self.num_blocks)
            self.W_histI[:n_hist, :] = self.ffe_motor._W_histI[:n_hist, :]
            self.W_histQ[:n_hist, :] = self.ffe_motor._W_histQ[:n_hist, :]
