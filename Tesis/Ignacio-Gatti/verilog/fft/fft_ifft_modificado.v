module fft_ifft #(
    ...
    parameter integer FINAL_INV_SCALE = 1  // 1: IFFT divide por N al final, 0: no divide
    ...
    wire signed [NB_W-1:0] outI_w =
        (i_inverse && FINAL_INV_SCALE) ? rshift_round_even(bufI[send_idx], LOGN) : bufI[send_idx];
    
    wire signed [NB_W-1:0] outQ_w =
    (i_inverse && FINAL_INV_SCALE) ? rshift_round_even(bufQ[send_idx], LOGN) : bufQ[send_idx];
